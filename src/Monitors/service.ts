...
export const primaryPollutant = ref<"pm25" | "o3">("pm25");
const monitors = ref<Record<string, Monitor>>({});
const activeMonitor = ref<Monitor>();
const monitorsMeta = ref<MonitorsMeta>();
...
export const useMonitorsService = asyncInitializer<MonitorsServiceModule>(
  async (resolve) => {
    ...
    monitorsMeta.value = await getMonitorsMeta();
    primaryPollutant.value = monitorsMeta.value.default_pollutant;
