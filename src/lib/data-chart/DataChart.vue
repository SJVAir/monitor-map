...
<template>
  <div
    class="backdrop is-flex is-flex-direction-column is-align-items-center is-justify-content-center"
    :class="{ visible: chartExpanded }"
  >
    <div
      :class="{ expanded: chartExpanded }"
      class="data-control is-flex is-flex-direction-column is-align-items-center card pb-4 pt-2"
    >
      <span
        v-if="!chartExpanded"
        class="expand icon is-clickable fullscreen-svg"
        title="Expand Chart"
        @click="toggleChart"
      ></span>
      <span
        v-else
        translate="no"
        class="expand icon is-clickable material-symbols-outlined"
        v-on:click="toggleChart"
        >close</span
      >

      <div
        class="date-control mt-2 is-flex is-align-items-center is-justify-content-space-evenly"
      >
        <div class="control">
          <label for="startDate" class="label is-small has-text-weight-normal"
            >Date Range</label
          >
          <DatePickerVue
        ...
        <div class="control">
          <br />
          <button
            class="button is-small is-info is-size-7 has-text-weight-semibold"
            v-on:click="async () => await loadChartData()"
          >
            <span class="icon is-small">
              <span
                translate="no"
                :class="{ spin: chartDataLoading }"
      ...
      <div class="download control is-align-self-flex-end">
        <button
          class="button is-small is-success is-size-7 has-text-weight-semibold"
          v-on:click="csvDownload"
        >
          <span class="icon is-small">
