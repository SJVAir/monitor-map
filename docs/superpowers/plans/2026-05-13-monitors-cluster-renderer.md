# Extract MonitorsClusterRenderer Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extract the cluster-specific MapTiler map manipulation from `MonitorsMapIntegration` into a plain `MonitorsClusterRenderer` class, leaving the integration file focused on reactive data derivation and orchestration.

**Architecture:** A new plain TypeScript class `MonitorsClusterRenderer` receives a `MonitorsClusterContext` interface (satisfied structurally by the integration) and exposes four imperative sync methods plus `apply`/`remove`. The integration instantiates the renderer as a private field, calls its methods from existing `$effect`s and `apply`/`remove`, and is otherwise unchanged in its reactive structure.

**Tech Stack:** Svelte 5 runes, MapTiler SDK, TypeScript
...
| ------ | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Create | `src/lib/monitors/monitors-cluster-renderer.ts`                    | All cluster-specific MapTiler source/layer manipulation; exports `MonitorsClusterContext`, `MonitorsClusterRenderer`, `monitorTooltip`           |
| Modify | `src/lib/monitors/monitors-map-integration.svelte.ts`              | Remove cluster private methods and `_clusterTypes`; add `renderer` field; delegate to renderer from effects and `apply`/`remove`; update imports |
| Modify | `src/lib/collocation-sites/collocations-map-integration.svelte.ts` | Remove dead `clustered` field                                                                                                                    |

---

### Task 1: Create `monitors-cluster-renderer.ts`

**Files:**

- Create: `src/lib/monitors/monitors-cluster-renderer.ts`

No test framework is configured — verify correctness by running `npm run check` after each task.

- [ ] **Step 1: Create the file with the complete implementation**

Write `src/lib/monitors/monitors-cluster-renderer.ts` with these exact contents:
...
git commit -m "feat: add MonitorsClusterRenderer"
```

---

### Task 2: Rewrite `monitors-map-integration.svelte.ts`
...
git commit -m "refactor: delegate cluster logic to MonitorsClusterRenderer"
```

---

### Task 3: Remove dead `clustered` field from `CollocationSitesMapIntegration`
...
In `collocations-map-integration.svelte.ts`, delete this line from the class body:
...
git commit -m "refactor: remove unused clustered field from CollocationSitesMapIntegration"
```
